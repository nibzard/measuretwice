...
| Runtime adversarial | `packages/measuretwice/test/runtime-adversarial.test.ts` | Vitest | The conformance and adversarial checks of task T077, each combining hostile conditions in one run through the public path: the total deadline ending one run that holds one retrying check that failed once and then answered one undeclared label, one delayed attempt, and two queue-full skips; cancellation during one backoff under one zero pending limit, with the disarmed backoff proven by the clock; repeated late callbacks against the frozen terminal report of the deadline, cancellation, and completion paths, with every wake-up disarmed; two cases assessed concurrently through one adapter, where every dispatched request holds the projection of its own case and no identifier, baseline, or foreign content crosses; one unauthorized input field and one input above its declared bound refusing before any evaluator runs; embedded instructions inside evidence and one command inside one rule input staying data that changes no question and no projection; one evidence state above the Jev budget recording one operational error with the stable code inside its message while the provider boundary receives no call; one unvalidated profile reaching no enforcement run with or without its own hash selected, one changed resolved model refusing with `model_resolution_changed`, and one edited model alias producing one new artifact whose hash the reviewed selection no longer names; one workflow of one shadow run, one evaluation, and one calibration over one host state, where the host decision, the host action ledger, and the stored profile bytes stay unchanged and every later enforcement run still needs its own selection; and the shared fixtures through the three shipped adapters, where the exact definition answers its trace case with no evaluator registered, the Jev adapter normalizes the pinned provider fixture through one recorded call, and the label-only adapter receives the same request and decides the same answer through the separately specified test rule. |
...
| Calibrate API | `packages/measuretwice/test/calibrate.test.ts` | Vitest | The calibrate operation of task T050: the plan and the dataset read through the bounded readers, the definition and evaluator checks of the plan before one dataset is read and the dataset checks before one case is measured, the measurement of both splits through the same validated execution path with no reference label in any evaluator request, the search over the development assessments, the frozen validation on the independent split, and the candidate profile with its complete evidence, its frozen policy, its measured performance with the stated sample minimums, and its computed qualification; the successful qualification with `validated_for_scope`, the missing evidence below the plan minimum with `insufficient_evidence`, the unmet goal with `criteria_not_met`, the no-candidate result that spends no validation budget and records the objective-best candidate, the cancellation refusals before the first case and during one measurement, the evaluator failure with the record's own code, the two resolved model versions with `model_resolution_changed`, the binding refusals before any spend, the calculation above the published fitting budget that refuses with its count, the complete-procedure option checks, the definition as one explicit path, the deterministic content hash of one repeated calibration, the retention statement that one ignored report folder holds no required copy of the evidence, the canary that no case content and no reviewer name crosses into the generated profile, and the closed boundary: the returned profile loads, shadow use admits it, and enforcement still needs the host selection and the matching scope. |
| Evidence check | `packages/measuretwice/test/evidence.test.ts` | Vitest | The `checkEvidence` operation of task T051: one real candidate profile built through `calibrate`, verified against the plan, the metadata, and the records that the calibration read, with every verified identity, the record counts of both splits, the recorded report references, and the standing limits; the drifted rows, one edited plan, one edited record file, one revised dataset revision, one edited profile copy, and one profile without evidence, each with its reason code and field path; the option checks over the explicit host-managed locations, including one wrong path format; the proof that the check reads no report file and changes no artifact; and the canary that no case content, no reference label, and no credential crosses into the result. |
| Calibration failures | `packages/measuretwice/test/calibrate-failures.test.ts` | Vitest | The failure cases of task T053 over the complete fitting and validation boundary, each through the public `calibrate` operation: the validation that accepts no case, with its `zero_denominator` goal, its null value, and its interval reason; the zero-error validation whose Wilson upper bound over three draws exceeds the declared limit while the same bound over ten fitting draws stayed inside it, so zero observed errors bound no zero risk; the absent labels that drop the labeled denominator below the plan minimum while the goal row keeps its one labeled accepted case; the required slice with no case and the slice below its floor, beside one met slice with its own metric set and interval rows; the dataset whose two splits share one group, refused at load before one evaluator call; the correlated validation groups that state `unsupported_sampling` under `independent_cases` while `grouped_cases` validates the same measurements; the synthetic challenge set and the reused holdout, both classified as development data that needs fresh evidence while the measured goals stay visible; the candidate tie that takes the first candidate of the declared order; the grid over the published decision budget, refused with its counts after the measurements; the conflicting goals that leave no feasible candidate with every limit unchanged and the validation budget unspent; and the abort during the validation measurements, refused with `run_cancelled` and no artifact. The statistics integrity: the workflow reports equal the direct `fitPolicy` and `qualifyCandidate` operations over the same stored assessments; every rate, goal, and bound traces to recorded counts, the declared basis, and the declared method, through one independent replay of the published decision rule and one independent Wilson formula; and provider confidence and baseline agreement never substitute for measured correctness, because one confidently wrong acceptance counts its reference error, one changed confidence changes no count, and one changed recorded decision changes the identities alone. |
| Revise API | `packages/measuretwice/test/revise.test.ts` | Vitest | The policy revision: one real prior calibration, reuse over one fresh validation split and over the consumed holdout, the new profile with its own content hash, the unchanged prior artifact, the revision comparison with its concrete changed cases, the reuse identity refusals, and the promotion gate that still refuses one stale qualification. |
...
| CLI | `packages/measuretwice/test/cli.test.ts` and `packages/measuretwice/test/cli-files.test.ts` | Vitest | The CLI surface of task T054: the compiled entry point through child processes and the imported `runCli` with injected streams; the parsing of every command with its resolved artifact paths, the `.measuretwice` name resolution, and the default metadata path of one records file; the usage failures with exit code 2 and their stable reason codes (`unknown_command`, `unsupported_option`, `missing_argument`, `unexpected_argument`, `invalid_argument`), the wrong-format paths with exit code 1 and `unsupported_format` before one read; the exit codes 0, 1, and 2 with stdout reserved for results and stderr for diagnostics, in text form and as one JSON error object under `--format json`; the credential canaries, where one credential option is one unsupported option, one credential variable never leaks, and one artifact field fails through the core; and the bounded readers, where the size bound fires before and after one read with `oversized_input` and no truncation, one unreadable path keeps its cause, malformed JSON fails with its position and no content echo, definitions, profiles, and cases keep the reason codes and field paths of the core, one executable field and one credential field fail with `unknown_field`, one edited profile fails with `hash_mismatch`, one unregistered evaluator reference fails with `evaluator_mismatch`, and the plan and report readers gate the frozen structure. The commands of task T055: `validate` states the meaning of one exported definition in both formats, with the expanded scale acceptance and the kinds of the core, and refuses malformed, hostile, and unreadable files; `run` renders one passing and one failing report with exit code 0, keeps one queue-full skip visible as one review outcome, prints and writes the report artifact through `--out` with `unwritable_output` on one failed write and no result on stdout, leaks no raw case content on either stream, keeps the reason code and the field path of one invalid case, refuses the question check with `evaluator_mismatch` and the CLI evaluator boundary with and without one profile, refuses one profile of another definition with `definition_mismatch`, and refuses `--mode enforcement` with `profile_not_selected` and the host-selection boundary; `inspect` states the readable summary, adds the bindings, the policy, the execution limits, and the identity on `--detail detailed`, prints the stored artifact under `--format json`, and refuses one edited profile with `hash_mismatch`. The commands of task T056: `calibrate` keeps the complete plan contract, the definition binding, and the evaluator refusal of the core with the CLI boundary sentence, writes no candidate profile, and refuses the exact-only definition with `policy_mismatch`, one plan of another definition with `definition_mismatch`, one edited signed plan with `hash_mismatch`, one broken grid with its field rule, one incomplete plan with `missing_field`, and one unreadable plan with `unreadable_file`; `evaluate` measures one labeled dataset of an exact-only definition through the same path as the library, prints the readable metric view and the JSON artifact with the default purpose `exploration` and the declared `fitting` limit, writes the report artifact through `--out` with `unwritable_output`, no result on stdout, and no artifact left behind, refuses one question check with `evaluator_mismatch` and the boundary sentence with and without one profile, refuses unreadable, malformed, foreign-label, and empty datasets, leaks no raw case content on either stream, and keeps one queue-full skip visible in the written report while the command still completes with exit code 0; `compare` renders the matching, the changed, and the metric rows of two stored reports with their counts and denominators in both formats, writes the comparison artifact through `--out`, and refuses one report of another definition with `definition_mismatch`, one disjoint case set with `insufficient_evidence`, one malformed file with `invalid_json`, and one edited stored count through the rebuilt arithmetic. One run of an evaluator-backed check ends in one error aggregate; the CLI refuses evaluator runs by design, so its suite covers the pass, fail, and review aggregates and the renderer suite of T037 covers the error view. |
| CLI workflow | `packages/measuretwice/test/cli-workflow.test.ts` | Vitest | The complete packaged journey of task T057: one isolated project holds the `.measuretwice` convention, the trusted application script authors and exports two definitions through `defineChecks`, the scripted test evaluator generates the exploration profiles and measures the stored evaluation reports of the question definition through the library, and every command of MVP_SPEC.md section 11 runs inside the project through the compiled entry point — `validate` on the exported artifact with the hash of the library path, `run` through the explicit paths of the section and the convention names with one pass, one fail, the report artifact of `--out`, and the refusals of the question check, the changed evaluator, and enforcement, `calibrate` on the convention plan with the evaluator boundary and no written candidate, `evaluate` on the convention datasets with the stored baseline and candidate reports, `compare` on the stored dataset reports and on the measured policy revision with its one changed case, and `inspect` on the structural exact, the exploration, and the candidate profiles at both levels and as the stored artifact. The artifact handling around the commands: absent convention names, wrong formats with one TypeScript body that never runs, malformed JSON with its position, failed output writes with no artifact left behind, the exit codes 0, 1, and 2 with every JSON result parsed independently of the terminal formatting, hostile `script`, `tool`, `permission`, and `plugin` fields on every artifact kind that reject as data through the core with nothing executed, the canaries that no label, no credential, and no raw input crosses any captured stream, and one armed offline guard with one positive control that refuses and records every network entry point of Node, so the complete journey proves it opens no connection and spends no API budget. |
| Memory support example | `packages/measuretwice/test/example-memory-support.test.ts` | Vitest | The public example of task T058, executed offline: the suite builds `examples/memory-support` through its own TypeScript configuration, as one host application does, and runs it against the scripted test evaluator. It checks the trusted import of the definition, the three synthetic references with their model-proposed provenance, the projected inputs of every evaluator request, the explicitly unvalidated exploration profile with its starter policy, the recorded baseline beside the unchanged host decision, the host-owned storage of the profile and every report, the private-data defaults that keep case content out of every stored report, the printed summary with its disagreement and its limits, the printed detailed view of one report with its key from task T076, and the enforcement refusal before any case work. |
...
| Plan review example | `packages/measuretwice/test/example-plan-review.test.ts` | Vitest | The portability example of task T068, executed offline: the suite builds `examples/plan-review`, the second application that shares no domain with the other examples, through its own TypeScript configuration and runs it against the scripted test evaluator. It checks the definition with its two Choice checks, one Noul check, one Score check, and one exact `includes` rule, the committed JSON export regenerated byte for byte by the trusted script and validated by the CLI, the two dataset revisions with one byte-identical fitting split, one consumed holdout, and one fresh validation split of grouped engagements, every evaluator request of every phase of one workflow that revisits the same cases with only the authorized inputs of its `using` list, the recorded baselines beside the unchanged host decisions, the aggregate fold recomputed for every report and re-verified through the shared renderer, the evaluation under the exploration profile, the calibration that selects the 0.6 cutoff with its measured goal and objective counts and its `insufficient_evidence` qualification from one development fixture, the revision that replays the stored fitting assessments, measures the fresh split, freezes the 0.8 cutoff, and names the two changed cases, the evidence check over the retained plan and dataset, the host-owned storage with no case content in any stored artifact, the printed summary with its disagreements and its limits, and the enforcement refusal of the exploration profile and of the revised profile even with one stated selection. |
...
| Reference documents | `tests/repo/reference-docs.test.ts` | Vitest | The published pages of `docs/reference`: every fenced TypeScript example compiles against the built package through one generated project under `build/`, every imported value name exists on the compiled entry point, every CLI option that a page names appears in the compiled usage text, every schema file that a page names exists in `contracts/v0`, and every fenced JSON block parses. The suite needs `npm run build` first. |
...
The package tests and the reference-document checks load the native binding
and the compiled entry points. Run `npm run build` before you run them.
...
- The unit tests of `crates/measuretwice-core/src/qualification.rs` hold
  ...
  the validation holds no case of, with the interval rows of every slice
  computed from the cases of that slice alone under its own sampling
  evidence and the empty slice stating no interval that implies observed
  cases, the empty validation split, the freeze
...
- The unit tests of `crates/measuretwice-core/src/profile.rs` hold the
  retained-evidence check of task T051 over one built plan, dataset, and
  signed calibration profile: the verified row with every computed
  identity and its record counts, the refusals of one edited plan, one
  foreign plan identifier, one swapped definition binding of the profile,
  one edited record, one revised or renamed dataset, one recorded split
  that the dataset declares nowhere, one recorded split set that omits a
  selection of the plan, and one profile without evidence, each with its
  reason code and field path. The artifact rejection table also covers the
  `performance.sample_minimums` field.
...
Task T076 ran the usability test of MVP_SPEC.md section 15 with four
restricted-reading sessions: two comprehension sessions over one check file
and one report, one integration session that reached one stored exploration
shadow report from `README.md` alone, and one agent-drafting session over
the published authoring guide. The record, the findings, and the fixes live
in [docs/reports/new-developer-test.md](docs/reports/new-developer-test.md).

The sessions were agent sessions, not human participants. The record states
that limit, and one human session remains open. The fixes that the test
produced are ordinary checked changes: the renderer key of the detailed
report view is covered by the rendering suite, and the example change is
covered by the memory support example suite.

## Runtime conformance and adversarial checks

Task T077 ran the runtime conformance and adversarial verification of
MVP_SPEC.md section 12: combined failure conditions in one run, repeated
late callbacks against terminal reports, cross-case isolation, untrusted
evidence, the enforcement gates of unvalidated and re-bound profiles, the
workflow boundary of shadow runs, evaluations, and calibrations, and the
shared fixtures through the exact, Jev-fixture, and label-only adapters.
The record, the observations, and the limits live in
[docs/reports/runtime-conformance.md](docs/reports/runtime-conformance.md).
The checks themselves are ordinary tests, so the runtime adversarial suite
of the table above keeps them green.
...
- Continuous integration does not run TLC. Task T078 decided the
  release-audit integration: the release audit of T079 reruns the three
  recorded commands, compares the state counts with each record, and
  treats one mismatch, one timeout, or one incomplete exploration as
  inconclusive.
- Task T078 verified the applicability of both models to the implemented
  state behavior on 25 September 2026. The mapping, the limited
  correspondence, and the limits live in
